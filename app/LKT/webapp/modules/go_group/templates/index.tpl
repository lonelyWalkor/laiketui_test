...
<div class="page-container pd-20 page_absolute">
  <div style="display: flex;flex-direction: row;font-size: 16px;" class="page_bgcolor">
    ...
    <div class="status qh {if $status==4}isclick{/if}"><a href="index.php?module=go_group&action=index&status=4" onclick="statusclick(4)">开团记录</a></div>
    <div class="status qh {if $status==5}isclick{/if}"><a href="index.php?module=go_group&action=index&status=5" onclick="statusclick(5)">参团记录</a></div>
    <div class="status qh {if $status==6}isclick{/if}"  style="border-radius: 0px 2px 2px 0px !important;"><a href="index.php?module=go_group&action=config" onclick="statusclick(5)">拼团设置</a></div>
  </div>
	...
	<div class="text-c" >
        {if  $status==4 || $status==5}
                ...
                <input type="text" name="proname" size='8' value="{$proname}" id="" placeholder="请输入商品名称" style="width:200px" class="input-text">
                <input type="text" name="username" size='8' value="{$username}" id="" placeholder="请输入会员名称" style="width:200px" class="input-text">
        ...
        {else}
            <form name="form1" action="index.php" method="get"  style="display: flex;align-items: center;">
                ...
                <input type="text" name="proname" size='8' value="{$proname}" id="" placeholder="请输入商品名称" style="width:200px" class="input-text" autocomplete="off">
                            ...
                            <div class="tab_block">
                                ...
                                <div>
                                    {if $item->g_status==1||$item->g_status==3}
                                        <a onclick="aj1({$item->group_id},'2','{$item->starttime}','{$item->endtime}','{$item->status}')" href="javascript:void(0);" title="开始">

                                            <img src="images/icon1/xj.png"/>&nbsp;开始

                                        </a>
                                    {else}
                                        <a onclick="aj1({$item->group_id},'3','{$item->starttime}','{$item->endtime}','{$item->status}')" href="javascript:void(0);" title="停止">
                                    ...
                                    {/if}
                                    {if $item->g_status!=2}
                                        <a title="删除" href="javascript:;" onclick="delgrouppro1(this,{$item->group_id},'index.php?module=go_group&action=member&m=delpro&id=')" class="ml-5" data-g_status="{$item->g_status}">
